...
ADodgeballCharacter::ADodgeballCharacter()
{
	...
	CameraBoom->bUsePawnControlRotation = false; // Rotate the arm based on the controller //false让摄像机不随鼠标动
	CameraBoom->SetRelativeRotation(FRotator(-70.0f, 0.f, 0.f));//设置摄像机Rotation（旋转）
	CameraBoom->bInheritPitch = false;
	CameraBoom->bInheritRoll = false;
	CameraBoom->bInheritYaw = false;//这三句让相机不跟随角色rotation变化而变化
